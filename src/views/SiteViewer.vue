...
// Le bouton retour (mobile/desktop) ne doit jamais servir à annuler un
// paiement (trop imprévisible une fois sur la page Stripe). Sur l'écran
// de paiement (svCartStep === "checkout"), on neutralise le bouton retour
// en repoussant systématiquement le même état : seul le bouton "✕ Annuler"
// explicite peut réellement annuler.
...
const payWithStripe = async () => {
  ...
  try {
    ...
    // On n'utilise plus jamais cfg.successUrl/cancelUrl (une valeur figée
    // enregistrée en base écrasait silencieusement ces paramètres critiques).
    ...
    <main class="sv-page" :style="currentPage?.style">
      <template v-for="s in currentPage?.sections" :key="s.id">
        ...
        <div v-else-if="s.type==='data'" class="sv-data-section" :style="s.style">
          ...
          <div v-if="getViewerDataState(s).sourceName" class="sv-data-result-title">Résultat</div>
...
/* DONNEES CHOISIES PAR LE VISITEUR */
.sv-data-section{padding:28px 32px}.sv-data-result-title{margin:6px 0 12px;font-size:18px;font-weight:700;color:var(--theme-text,#111827)}.sv-data-title{margin:0 0 16px;font-size:24px}.sv-data-picker{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px}.sv-data-btn{padding:10px 14px;border:1px solid #e5e7eb;border-radius:9px;background:#fff;color:#374151;font-weight:600;cursor:pointer}.sv-data-btn:hover{border-color:var(--theme-accent,#6c63ff);color:var(--theme-accent,#6c63ff)}.sv-data-source{font-size:13px;color:#6b7280}.sv-data-table-picker{display:flex;align-items:center;gap:8px;margin:8px 0 14px;font-size:13px}.sv-data-table-picker select{padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;background:#fff}.sv-data-status{color:#6b7280;font-size:13px}.sv-data-error{padding:10px 12px;border-radius:8px;background:#fef2f2;border:1px solid #fecaca;color:#b91c1c;font-size:13px}.sv-data-table-wrap{overflow:auto;border:1px solid #e5e7eb;border-radius:10px}.sv-data-table{width:100%;border-collapse:collapse;font-size:13px}.sv-data-table th,.sv-data-table td{padding:10px 12px;border-bottom:1px solid #f0f0f0;text-align:left;white-space:nowrap}.sv-data-table th{background:#f9fafb;font-weight:700}.sv-data-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px}.sv-data-card{padding:14px;border:1px solid #e5e7eb;border-radius:12px;background:#fff}.sv-data-card>div{display:flex;justify-content:space-between;gap:12px;padding:5px 0;border-bottom:1px solid #f3f4f6}.sv-data-card strong{font-size:12px;color:#6b7280}.sv-data-card span{font-size:13px;text-align:right;word-break:break-word}.sv-data-empty{padding:20px;border:1px dashed #d1d5db;border-radius:10px;color:#6b7280;text-align:center}.sv-data-pagination{display:flex;justify-content:center;align-items:center;gap:12px;margin-top:14px;font-size:13px}.sv-data-pagination button{padding:6px 10px;border:1px solid #e5e7eb;border-radius:7px;background:#fff;cursor:pointer}.sv-data-pagination button:disabled{opacity:.4;cursor:not-allowed}
</style>
