...
// Vérifie auprès de Stripe lui-même que le paiement a réellement abouti.
// Le jeton (isCurrentAttempt) protège contre une ANCIENNE page de succès ;
// cette vérification protège contre le cas où le navigateur atterrit sur
// CETTE URL de succès sans que l'argent ait réellement été prélevé (ex:
// la flèche "retour" de Stripe qui, selon les cas, peut y renvoyer quand
...
async function isReallyPaid() {
  ...
  try {
    ...
    console.warn("isReallyPaid:", e.message)
    // Impossible de vérifier (réseau, backend down) : par prudence on ne
    // bloque pas un vrai succès sur un simple souci réseau côté client.
    return true
  }
}
...
onMounted(async () => {
  ...
  const reallyPaid = await isReallyPaid()
  if (!reallyPaid) {
    verifyFailed.value = true
    saving.value = false
    return   // paiement non confirmé auprès de Stripe : pas de faux succès
  }
